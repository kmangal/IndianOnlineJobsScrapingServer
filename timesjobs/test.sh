...
nohup sh -c '{ logdate=$(date +"%Y%m%d"); starttime=$(date +"%m/%d/%Y %H:%M:%S %Z"); } ; \
	     { python3 -u all_city_main.py --debug && \
	       python3 -u export_to_dropbox.py test/log/${logdate}.log "/India Labor Market Indicators/scraping/TimesJobs/ec2/test/log/${logdate}.log"; } && \
	     { endtime=$(date +"%m/%d/%Y %H:%M:%S %Z"); \
	       echo -e "Subject: TimesJobs Completed Successfully\n\nLog file saved in India Labor Market Indicators/scraping/TimesJobs/ec2/test/log/${logdate}.log\nStart Time: ${starttime}\nEnd Time:  ${endtime}" | \
	     ...
	     { { echo "Subject: TimesJobs Scrape Failed"; cat test/log/${logdate}.log; } | \
	       /usr/sbin/sendmail -f india.labourinsights.server@gmail.com -F "India Labour Insights Server" india.labourinsights.server@gmail.com; }' \
	     > test/log/${logdate}.log &
exit 0
