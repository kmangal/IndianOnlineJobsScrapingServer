...
{ python3 timesjobs_main.py --mainpage=test/mainpage/timesjobs_mainpage_${filedate}.csv /
			    --jobcount=test/jobcount/timesjobs_jobcount_${filedate}.csv &&
  python3 timesjobs_details.py --input=test/mainpage/timesjobs_mainpage_${filedate}.csv /
			       --output=test/details/timesjobs_details_${filedate}.csv &&
...
{ 
	{ echo "Subject: TimesJobs Scrape Failed"; cat log/${filedate}.log; } | \
	/usr/sbin/sendmail -f india.labourinsights.server@gmail.com -F "India Labour Insights Server" india.labourinsights.server@gmail.com; 
}
